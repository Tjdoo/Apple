...
/*!
 * @const DISPATCH_SOURCE_TYPE_DATA_ADD  变量增加
 * @discussion A dispatch source that coalesces data obtained via calls to dispatch_source_merge_data(). An ADD is used to coalesce the data.
 * The handle is unused (pass zero for now).  The mask is unused (pass zero for now).
 * 当调用 dispatch_source_create 方法时， type = DISPATCH_SOURCE_TYPE_DATA_ADD，handle、mask 暂未使用，可以传 0
 */
...
/*!
 * @const DISPATCH_SOURCE_TYPE_DATA_OR  变量 OR
 * @discussion A dispatch source that coalesces data obtained via calls to dispatch_source_merge_data(). A bitwise OR is used to coalesce the data.
 * The handle is unused (pass zero for now).  The mask is unused (pass zero for now).
 * 当调用 dispatch_source_create 方法时， type = DISPATCH_SOURCE_TYPE_DATA_OR，handle、mask 暂未使用，可以传 0
 */
 ...
 * @discussion A dispatch source that tracks data obtained via calls to dispatch_source_merge_data(). Newly obtained data values replace existing
...
/*!
 * @const DISPATCH_SOURCE_TYPE_MACH_SEND      MACH 端口发送
 * @discussion A dispatch source that monitors a Mach port for dead name notifications (send right no longer has any corresponding receive right).
...
/*!
 * @const DISPATCH_SOURCE_TYPE_MACH_RECV    MACH 端口接收
...
/*!
 * @const DISPATCH_SOURCE_TYPE_MEMORYPRESSURE    内存压力（iOS8后可用）
 * @discussion A dispatch source that monitors the system for changes in memory pressure condition.
 * The handle is unused (pass zero for now).
 * The mask is a mask of desired events from dispatch_source_memorypressure_flags_t.
 */
...
/*!
 * @const DISPATCH_SOURCE_TYPE_PROC    检测到与进程相关的事件
 * @discussion A dispatch source that monitors an external process for events defined by dispatch_source_proc_flags_t.
...
/*!
 * @const DISPATCH_SOURCE_TYPE_READ    可读取文件映像
 * @discussion A dispatch source that monitors a file descriptor for pending bytes available to be read.
...
/*!
 * @const DISPATCH_SOURCE_TYPE_SIGNAL    接收信号
...
/*!
 * @const DISPATCH_SOURCE_TYPE_TIMER   定时器
 * @discussion A dispatch source that submits the event handler block based on a timer.
 * The handle is unused (pass zero for now).
...
/*!
 * @const DISPATCH_SOURCE_TYPE_VNODE     文件系统有变更
 * @discussion A dispatch source that monitors a file descriptor for events defined by dispatch_source_vnode_flags_t.
 * The handle is a file descriptor (int).
...
/*!
 * @const DISPATCH_SOURCE_TYPE_WRITE     可写入文件映像
 * @discussion A dispatch source that monitors a file descriptor for available buffer space to write bytes.
 * The handle is a file descriptor (int).
 * The mask is unused (pass zero for now).
 */
 ...
 * and only when absolutely necessary.  使用 DISPATCH_TIMER_STRICT，会引起电量消耗加剧，毕竟要求精确时间
 */
