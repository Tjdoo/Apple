...
/**
  *  @param   dst   设置 GCD 源方法的类型。
  *  @param   handle  根据各类型的描述，传入不同的值。传 0 或 mach_port_t 值等
  *  @param   mask     根据各类型的描述，传入不同的值。传0 或 dispatch_source_timer_flags_t 值等
  *  @param   dq   队列。将定时器事件处理的 Block 提交到哪个队列之上。可以传 Null，默认为全局队列。注意：当提交到全局队列的时候，事件处理的回调内，需要异步获取 UI 线程。
  *  @see   source.h
  */
dispatch_source_t
					   ...
					   dispatch_queue_t dq)
{
	...
	// (dst)->dst_create(dst, handle, mask)    -》  event.c 文件中不同的事件 dst_create()  对应不同的实现  -》  _dispatch_source_timer_create()
	dr = dux_create(dst, handle, mask)._dr;
	if (unlikely(!dr)) {
		return DISPATCH_BAD_INPUT;
	}

	// 为 dispatch_source_t 类型结构体 ds 分配内存，DISPATCH_VTABLE(source) -》OS_dispatch_##name##_class
...
DISPATCH_ALWAYS_INLINE
/**
  *  @brief   生成 dispatch_continuation_t 对象
  *  @param   ds
  *  @param   func   回调方法（处理源事件、取消源事件处理等）
  *  @param   kind   处理时间的类型：DS_EVENT_HANDLER、DS_REGISTN_HANDLER、DS_CANCEL_HANDLER
  *  @param
  */
static inline dispatch_continuation_t
...
/**
  *  @param   ds
  *  @param   handler   定时器执行的动作，需要处理的业务逻辑 Block。
  */
void
...
/**
  *  @param   ds
  *  @param   start   定时器开始时间，类型为 dispatch_time_t。其 API 的 abstract 表明可参照 dispatch_time() 和 dispatch_walltime()，同为设置时间，但是后者为“钟表”时间，相对比较准确，所以选择使用后者。dispatch_walltime(const struct timespec *_Nullable when, int64_t delta),参数when可以为Null，默认为获取当前时间，参数 delta 为增量，即获取当前时间的基础上，增加 X 秒的时间为开始计时时间，此处传 0 即可。dispatch_walltime(NULL, 0)
  *  @param   interval   定时器间隔时长，由业务需求而定。
  *  @param   leeway   允许的误差。
  */
DISPATCH_NOINLINE
void
